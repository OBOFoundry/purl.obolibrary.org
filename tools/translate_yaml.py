...
def write_entries(entries, yamlname, outfile):
  """
  Write the given entries to the given outfile, indicating the source YAML file
  from which the entries were extracted.
  """
...
def translate_products(yamldoc):
  ...
  if 'products' in yamldoc and type(yamldoc['products']) is list:
    products = []
    for i, product in enumerate(yamldoc['products']):
      products.append(process_product(i, product))
    return products


def append_products(products, idspace, outfile):
  """
  Appends the given list of products for the given idspace to the given output stream.
  """
  if products:
...
def append_term(term, idspace, outfile):
  """
  Appends the given term for the given idspace to the given output stream.
  """
  if term:
