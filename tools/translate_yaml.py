...
Note that when `term_browser: custom` is used no output is generated.
"""
...
def main():
  ...
  if args.input_files:
    ...
    with open('{}/.htaccess'.format(normalised_output_dir), 'a') as outfile:
      ...
      outfile.write('\n### Generated from adoptions.tsv\n\n')
      with open('{}/adoptions.tsv'.format(normalised_input_dir)) as f:
        rows = csv.DictReader(f, delimiter='\t')
        for row in rows:
          outfile.write("""RedirectMatch seeother "(?i)^/obo/{}$" "http://www.ontobee.org/browser/rdf.php?o={}&iri=http://purl.obolibrary.org/obo/{}"
""".format(row['ID'], row['IDSPACE'], row['ID']))

      outfile.write('\n### Generated from obo.yml\n\n')
      for entry in entries['OBO'] or []:
        outfile.write('{}\n'.format(entry))
